...
test('coach voice acknowledges played audio and releases the microphone on navigation',async({page})=>{
 ...
 await page.goto('/coach');await page.getByRole('button',{name:'Engineer · 2027-01-01'}).click();await page.getByRole('button',{name:'Connect to AI Coach for 10 minutes'}).click();await expect.poll(()=>acknowledged).toBe(true);await expect(page.getByText('Listening to you',{exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Mute microphone'})).toBeVisible();await expect(page.getByRole('button',{name:'End coaching session'})).toBeVisible();await page.getByRole('link',{name:'My profile',exact:true}).click();await expect.poll(()=>ended).toBe(true);await expect.poll(()=>page.evaluate(()=>(window as any).micReleased)).toBe(true);
});

test('overview placement actions carry the latest drive into coach and interview preparation',async({page})=>{
 await mockStudent(page);const drive={id:'drive-latest',company_name:'VoiceDot',company_description:'Conversational AI products for enterprise teams.',role_title:'AI Engineer',location:'Bengaluru',drive_date:'2027-01-15',status:'active'};
 await page.route('**/api/student/dashboard',route=>route.fulfill({json:{...dashboard,drives:[drive]}}));
 await page.goto('/');await expect(page.getByRole('heading',{name:'AI Engineer'})).toBeVisible();await expect(page.getByText('VoiceDot',{exact:true})).toBeVisible();
 await expect(page.getByRole('link',{name:/Prepare with AI Coach/})).toHaveAttribute('href','/coach?drive=drive-latest');
 await expect(page.getByRole('link',{name:/Take AI Interview/})).toHaveAttribute('href','/practice?drive=drive-latest');
});
