...
export function CoachVoice({planId,onComplete}:{planId:string;onComplete:()=>void}){
 ...
 return <section className="coach-voice"><h3>Talk to your coach</h3><p>Learn through a voice conversation. You can interrupt to ask a question.</p><p role="status">{status}</p><div className="career-actions"><label>Lesson length<select disabled={active} value={duration} onChange={e=>setDuration(e.target.value)}><option value="10">10 minutes</option><option value="20">20 minutes</option></select></label>{!active?<button className="button primary" onClick={()=>void start()}>Start voice lesson</button>:<><button className="button secondary" onClick={()=>{mute.current=!mute.current;setMuted(mute.current)}}>{muted?'Unmute microphone':'Mute microphone'}</button><button className="button secondary" onClick={()=>{generation.current++;cleanup.current();setActive(false);setStatus('Call ended. Completed turns are saved.');done.current()}}>End lesson</button></>}</div>{caption&&<p className="voice-caption">{caption}</p>}</section>
}
